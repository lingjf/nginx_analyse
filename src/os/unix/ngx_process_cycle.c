        ...
        if (ngx_processes[i].exited) {
            ...
            if (ngx_processes[i].pid == ngx_new_binary) {
                ...
                if (ngx_rename_file((char *) ccf->oldpid.data, (char *) ccf->pid.data) != NGX_OK) {
                    ngx_log_error(NGX_LOG_ALERT, cycle->log, ngx_errno, ngx_rename_file_n " %s back to %s failed " "after the new binary process \"%s\" exited", ccf->oldpid.data, ccf->pid.data, ngx_argv[0]);
                }
