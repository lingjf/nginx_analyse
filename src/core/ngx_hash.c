...
ngx_hash_init(ngx_hash_init_t *hinit, ngx_hash_key_t *names, ngx_uint_t nelts)
{
    ...
    /* 不向内存池申请内存，而直接向系统申请内存的原因是：这块内存是临时内存这个函数内用完后即无用需要释放。
     * 如果向内存池(ngx_cycle的内存池)申请的话，就不会释放了造成浪费。 */
    ...
    /* start 为最小bucket数量(HASH数组大小)
     * NGX_HASH_ELT_SIZE的最小值是 2 * sizeof(void *)
