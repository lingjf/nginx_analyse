...
ngx_conf_set_enum_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    ngx_conf_log_error(NGX_LOG_WARN, cf, 0, "invalid value \"%s\"", value[1].data);
...
u_char *jeff_module_type_tustring(unsigned long module_type)
{
   switch (module_type) {
   case NGX_CORE_MODULE: return (u_char*)"CORE";
   case NGX_CONF_MODULE: return (u_char*)"CONF";
   case 0x50545448: return (u_char*)"HTTP";
   case 0x4C49414D: return (u_char*)"MAIL";
   default:
      return (u_char*)"NULL";
   }
}

u_char *jeff_command_type_tustring(unsigned long command_type)
{
   ...
   if ((command_type & NGX_ANY_CONF) == NGX_ANY_CONF)
      ngx_snstrcatf(buffer, sizeof(buffer), "Any");
   else {
      if (command_type & NGX_MAIN_CONF)
         ngx_snstrcatf(buffer, sizeof(buffer), "Main");
      if (command_type & 0x02000000)
         ngx_snstrcatf(buffer, sizeof(buffer), "Httpmain");
      if (command_type & 0x04000000)
         ngx_snstrcatf(buffer, sizeof(buffer), "Httpsrv");
      if (command_type & 0x08000000)
         ngx_snstrcatf(buffer, sizeof(buffer), "Httploc");
      if (command_type & 0x10000000)
         ngx_snstrcatf(buffer, sizeof(buffer), "Httpups");
      if (command_type & 0x20000000)
         ngx_snstrcatf(buffer, sizeof(buffer), "Httpsif");
      if (command_type & 0x40000000)
         ngx_snstrcatf(buffer, sizeof(buffer), "Httplif");
      if (command_type & 0x80000000)
         ngx_snstrcatf(buffer, sizeof(buffer), "Httplmt");
   }

   /* command flags */
   ngx_snstrcatf(buffer, sizeof(buffer), "-");
   if (command_type & NGX_CONF_BLOCK)
      ngx_snstrcatf(buffer, sizeof(buffer), "Block");
   if (command_type & NGX_CONF_FLAG)
      ngx_snstrcatf(buffer, sizeof(buffer), "Flag");
   if (command_type & NGX_CONF_ANY)
      ngx_snstrcatf(buffer, sizeof(buffer), "Any");
   if (command_type & NGX_CONF_1MORE)
      ngx_snstrcatf(buffer, sizeof(buffer), "1more");
   if (command_type & NGX_CONF_2MORE)
      ngx_snstrcatf(buffer, sizeof(buffer), "2more");
   if (command_type & NGX_DIRECT_CONF)
      ngx_snstrcatf(buffer, sizeof(buffer), "Direct");

   /* argurments numbers */
   ngx_snstrcatf(buffer, sizeof(buffer), "-");
   if (command_type & NGX_CONF_NOARGS)
      ngx_snstrcatf(buffer, sizeof(buffer), "0");
   if (command_type & NGX_CONF_TAKE1)
      ngx_snstrcatf(buffer, sizeof(buffer), "1");
   if (command_type & NGX_CONF_TAKE2)
      ngx_snstrcatf(buffer, sizeof(buffer), "2");
   if (command_type & NGX_CONF_TAKE3)
      ngx_snstrcatf(buffer, sizeof(buffer), "3");
   if (command_type & NGX_CONF_TAKE4)
      ngx_snstrcatf(buffer, sizeof(buffer), "4");
   if (command_type & NGX_CONF_TAKE5)
      ngx_snstrcatf(buffer, sizeof(buffer), "5");
   if (command_type & NGX_CONF_TAKE6)
      ngx_snstrcatf(buffer, sizeof(buffer), "6");
   if (command_type & NGX_CONF_TAKE7)
      ngx_snstrcatf(buffer, sizeof(buffer), "7");
...
u_char *jeff_conf_tustring(ngx_conf_t *c)
{
   ...
   ngx_snprintf(buffer, sizeof(buffer), "ngx_conf_t[%s %s %s]",
                ...
                jeff_array_tustring(c->args, jeff_str_tustring));
   return buffer;
}
