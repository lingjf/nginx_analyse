...
#ifdef UNITEST
int ngx_cdecl main_unitest(int argc, char *const *argv)
#else
...

{
    ...
    if (ngx_show_version) {
        ...
        if (ngx_show_help) {
            ngx_write_stderr(
                "Usage: nginx [-?hvVtq] [-s signal] [-c filename] " "[-p prefix] [-g directives]" NGX_LINEFEED NGX_LINEFEED
...
ngx_set_cpu_affinity(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    for (n = 1; n < cf->args->nelts; n++) {
        ...
        for (i = 0; i < value[n].len; i++) {
            ...
            ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "invalid character \"%c\" in \"worker_cpu_affinity\"",
                          ch);
            return NGX_CONF_ERROR;
        }
    }
