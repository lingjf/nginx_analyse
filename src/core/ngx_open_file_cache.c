...
ngx_open_file_cache_cleanup(void *data)
{
    ...
    if (cache->current) {
        ngx_log_error(NGX_LOG_ALERT, ngx_cycle->log, 0, "%d items still leave in open file cache", cache->current);
    }
...
static void
ngx_close_cached_file(ngx_open_file_cache_t *cache, ngx_cached_open_file_t *file, ngx_uint_t min_uses, ngx_log_t *log)
{
...
ngx_expire_old_cached_files(ngx_open_file_cache_t *cache, ngx_uint_t n, ngx_log_t *log)
{
     ...
     * n == 0 deletes least recently used file by force and one or two inactive files
     */
...
static void
ngx_open_file_cache_rbtree_insert_value(ngx_rbtree_node_t *temp, ngx_rbtree_node_t *node, ngx_rbtree_node_t *sentinel)
{
    ngx_rbtree_node_t       **p;
...
u_char *jeff_open_file_cache_tustring(ngx_open_file_cache_t *c)
{
   static u_char buffer[1024 * 8];
   memset(buffer, 0, sizeof(buffer));
   if (c == NULL) return "NULL";
   ngx_snprintf(buffer, sizeof(buffer), "ngx_open_file_cache_t[%d/%d,%T]",
                c->current, c->max, c->inactive);
   return buffer;
}

u_char *jeff_cached_open_file_tustring(ngx_cached_open_file_t *f)
{
   static u_char buffer[1024 * 8];
   memset(buffer, 0, sizeof(buffer));
   ngx_snprintf(buffer, sizeof(buffer), "ngx_cached_open_file_t[%s,%d]",
                f->name, f->size);
   return buffer;
}
