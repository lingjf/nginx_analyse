...
static ngx_int_t ngx_http_script_init_arrays(ngx_http_script_compile_t *sc)
{
   ...
   if (*sc->values == NULL) {
      n = (sc->variables * (2 * sizeof(ngx_http_script_copy_code_t) + sizeof(ngx_http_script_var_code_t)) + sizeof(uintptr_t) + sc->source->len + sizeof(uintptr_t) - 1) & ~(sizeof(uintptr_t) - 1);
...
static void ngx_http_script_full_name_code(ngx_http_script_engine_t *e)
{
   ...
   if (ngx_conf_full_name((ngx_cycle_t *) ngx_cycle, &value, code->conf_prefix) != NGX_OK) {
      e->ip = ngx_http_script_exit;
      e->status = NGX_HTTP_INTERNAL_SERVER_ERROR;
      return;
   }
