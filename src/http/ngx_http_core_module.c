...
ngx_http_core_run_phases(ngx_http_request_t *r)
{
    ...
    while (ph[r->phase_handler].checker) {
        if (ph[r->phase_handler].checker(r, &ph[r->phase_handler]) == NGX_OK) {
            return;
        }
    }
}
...
ngx_http_core_try_files_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)
{
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "try files phase: %ui", r->phase_handler);
...
ngx_http_update_location_config(ngx_http_request_t *r)
{
    ...
    if (r->keepalive) {
        if (clcf->keepalive_timeout == 0) {
            ...
            /* MSIE may wait for some time if an response for a POST request was sent over a keepalive connection */
            r->keepalive = 0;
        } else if (r->headers_in.safari && (clcf->keepalive_disable & NGX_HTTP_KEEPALIVE_DISABLE_SAFARI)) {
            /* Safari may send a POST request to a closed keepalive connection and may stall for some time, see https://bugs.webkit.org/show_bug.cgi?id=5760 */
            r->keepalive = 0;
        }
    }
...
/*
 * Jeff: Configuration callback start ...
 */
...
ngx_http_core_type(ngx_conf_t *cf, ngx_command_t *dummy, void *conf)
{
    ...
    if (ngx_strcmp(value[0].data, "include") == 0) {
        if (cf->args->nelts != 2) {
            ngx_conf_log_error(NGX_LOG_EMERG, cf, 0, "invalid number of arguments" " in \"include\" directive"); return NGX_CONF_ERROR;
        }
...
ngx_http_core_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    ngx_conf_merge_uint_value(conf->types_hash_bucket_size, prev->types_hash_bucket_size, ngx_cacheline_size);
...
u_char *jeff_http_core_loc_conf_tustring(ngx_http_core_loc_conf_t *l)
{
   static u_char buffer[1024 * 8];
   memset(buffer, 0, sizeof(buffer));
   ngx_snstrcatf(buffer, sizeof(buffer), "ngx_http_core_loc_conf_t:\n"
                 "\t name=%V\n"
                 "\t root=%V\n" ,
                 &l->name,
                 &l->root);
