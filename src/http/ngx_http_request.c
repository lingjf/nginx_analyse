        ...
        if (rc == NGX_OK) {
            ...
            while (p < last) {
                ...
                if (*p++ == ':') {
                    ...
                    if (p < last && *p == '$') {
                        ngx_log_error(NGX_LOG_INFO, c->log, 0, "client sent unsafe win32 URI");
                        ngx_http_finalize_request(r, NGX_HTTP_BAD_REQUEST);
                        return;
                    }
                }
...
static ngx_int_t
ngx_http_process_user_agent(ngx_http_request_t *r, ngx_table_elt_t *h, ngx_uint_t offset)
{
...
static ngx_int_t
ngx_http_process_cookie(ngx_http_request_t *r, ngx_table_elt_t *h, ngx_uint_t offset)
{
...
u_char *jeff_http_request_tustring(ngx_http_request_t *r)
{
   ...
   ngx_snstrcatf(buffer, sizeof(buffer), "ngx_http_request_t:\n"
                 "\t request_line=%V\n"
                 "\t uri=%V\n"
                 "\t args=%V\n",
                 &r->request_line,
