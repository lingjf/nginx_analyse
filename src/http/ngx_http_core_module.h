...
typedef struct {
    ...
    unsigned                    listen:1; /* flag for have 'listen' directive */
#if (NGX_PCRE)
    unsigned                    captures:1;
#endif
...
ngx_int_t ngx_http_get_forwarded_addr(ngx_http_request_t *r, ngx_addr_t *addr, u_char *xff, size_t xfflen, ngx_array_t *proxies, int recursive);
