...
static void ngx_http_upstream_free_least_conn_peer(ngx_peer_connection_t *pc, void *data, ngx_uint_t state);
...
static ngx_int_t
ngx_http_upstream_init_least_conn(ngx_conf_t *cf, ngx_http_upstream_srv_conf_t *us)
{
    ngx_uint_t                            n;
    ...
    lcf = ngx_http_conf_upstream_srv_conf(us, ngx_http_upstream_least_conn_module);
...
static ngx_int_t
ngx_http_upstream_init_least_conn_peer(ngx_http_request_t *r, ngx_http_upstream_srv_conf_t *us)
{
...
static void
ngx_http_upstream_free_least_conn_peer(ngx_peer_connection_t *pc, void *data, ngx_uint_t state)
{
    ngx_http_upstream_lc_peer_data_t  *lcp = data;

    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, pc->log, 0, "free least conn peer %ui %ui", pc->tries, state);
...
ngx_http_upstream_least_conn_create_conf(ngx_conf_t *cf)
{
    ...
    conf = ngx_pcalloc(cf->pool, sizeof(ngx_http_upstream_least_conn_conf_t));
    if (conf == NULL) {
        return NULL;
    }
