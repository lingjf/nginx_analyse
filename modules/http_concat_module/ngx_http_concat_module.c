...
static ngx_int_t ngx_http_concat_add_path(ngx_http_request_t *r, ngx_array_t *uris, ngx_str_t *path, u_char *p, u_char *v);
...
ngx_http_concat_handler(ngx_http_request_t *r)
{
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http concat root: \"%V\"", &path);
    ...
    for (p = r->args.data + 1, v = p, timestamp = 0; p != e; p++) {
        ...
        if (*p == ',') {
            ...
            rc = ngx_http_concat_add_path(r, &uris, &path, p, v);
            if (rc != NGX_OK) {
                return rc;
            }
    ...
    for (i = 0; i < uris.nelts; i++) {
        ...
        if (ngx_open_cached_file(ccf->open_file_cache, filename, &of, r->pool) != NGX_OK) {
            ...
            if (rc != NGX_HTTP_NOT_FOUND || ccf->log_not_found) {
                ngx_log_error(level, r->connection->log, of.err, "%s \"%V\" failed", of.failed, filename);
            }
        ...
        if (!of.is_file) {
            ngx_log_error(NGX_LOG_CRIT, r->connection->log, ngx_errno, "\"%V\" is not a regular file", filename);
...
static ngx_int_t
ngx_http_concat_add_path(ngx_http_request_t *r, ngx_array_t *uris, ngx_str_t *path, u_char *p, u_char *v)
{
    u_char     *d;
    ngx_str_t  *uri, args;
    ngx_uint_t  flags;
    ...
    if (p == v) {
        ngx_log_error(NGX_LOG_ERR, r->connection->log, 0, "client sent zero concat filename");
    ...
    *d = '\0';
    ...
    ngx_log_debug1(NGX_LOG_DEBUG_HTTP, r->connection->log, 0, "http concat add file: \"%s\"", uri->data);
